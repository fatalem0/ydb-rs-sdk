use ydb::{ydb_params, ClientBuilder, Query, TableClient, YdbResult};
use ydb_slo_tests::cli::SloTestsCli;
use ydb_slo_tests::row::{Row, RowID};
...
//
//         let upsert_query = Query::new(format!(
//             r#"
...
pub struct Db {
    ...
    pub(crate) cli_args: SloTestsCli,
    upsert_query: Query,
    select_query: Query,
}
...
impl Db {
    pub async fn new(cli: SloTestsCli) -> YdbResult<Self> {
        ...
        let upsert_query = Query::from(format!(
            r#"
PRAGMA TablePathPrefix("{}");
...
"#,
            &cli.db, &cli.table_name
        ));

        let select_query = Query::from(format!(
            r#"
PRAGMA TablePathPrefix("{}");
...
"#,
        ...
        Ok(Self {
            ...
            cli_args: cli,
            upsert_query: upsert_query.clone(),
    ...
    pub async fn read_row_by_id(&self, row_id: RowID) -> YdbResult<()> {
        let query = self
            .select_query
            .clone()
    ...
    pub async fn insert_row(&self, row: Row) -> YdbResult<()> {
        let query = self.clone().upsert_query.with_params(ydb_params!(
                "$id" => row.id,
                "$payload_str" => row.payload_str,
                "$payload_double" => row.payload_double,
                "$payload_timestamp" => row.payload_timestamp.unwrap(), //TODO: поменять
        ));
